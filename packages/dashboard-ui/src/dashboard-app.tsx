...
import type {
  ...
  DashboardRun, DashboardRunComparison, DashboardRunPage,
  DashboardScannerStatus, DashboardTraceInsight,
...
import { TokenTraceView } from "./token-trace-view";
...
function RunTable(props: { data: DashboardRunPage; locale: DashboardLocale; route: DashboardRoute; navigate: DashboardNavigate; selected: string[]; setSelected: (ids: string[]) => void; deleteOne: (id: string) => void }) {
  ...
  return <div className="at-table-wrap"><table className="at-table at-run-table"><colgroup><col style={{ width: 42 }} /><col style={{ width: 330 }} /><col style={{ width: 135 }} /><col style={{ width: 105 }} /><col style={{ width: 145 }} /><col style={{ width: 175 }} /><col style={{ width: 145 }} /><col style={{ width: 125 }} /><col style={{ width: 140 }} /><col style={{ width: 100 }} /><col style={{ width: 60 }} /></colgroup><thead><tr>
    ...
    <td><a href={`/runs/${encodeURIComponent(run.id)}`} onClick={linkClick(navigate, `/runs/${encodeURIComponent(run.id)}`)}><strong className="at-run-name" title={run.name}>{normalizeRunTitle(run.name) || run.id}</strong><div className="at-subtle at-mono" title={run.id}>{run.id}</div></a></td>
    <td><SourceBadge source={sourceLabel(locale, agent)} /><div className="at-subtle">{surfaceLabel(locale, run.metadata?.surface || run.metadata?.source || "local")}</div></td>
...
export function RunDetailView(context: PageContext & { id: string }) {
  ...
  return <>
    <PageHead locale={locale} eyebrow={tr(locale, "追踪详情", "TRACE DETAIL")} title={normalizeRunTitle(run.name) || run.id} body={run.id} />
    ...
    <TelemetryStrip items={[[tr(locale, "状态", "Status"), statusLabel(locale, run.status), ""], [tr(locale, "事件", "Events"), fmt(events.counts.total), ""], ["Token", fmt(summary?.tokenUsage.total || events.summary.totalTokens), ""], [tr(locale, "成本", "Cost"), formatMoney(summary?.costUsd || 0), formatDuration(events.summary.totalDurationMs)]]} />
    <div className="at-grid-2"><section className="at-card"><div className="at-card-head"><h2>{tr(locale, "自动诊断", "Automatic diagnostics")}</h2></div>{insights.insights.length ? <div className="at-list">{insights.insights.map(item => <div className="at-list-item" key={`${item.kind}-${item.eventIds.join()}`}><strong>{item.title}</strong><p>{insightKindLabel(locale, item.kind)} · {severityLabel(locale, item.severity)} · {item.eventIds.length} {tr(locale, "个事件", "events")}</p><div className="at-actions">{item.eventIds.slice(0, 4).map(eventId => <a className="at-badge" href={`#event-${eventId}`} key={eventId}>{eventId.slice(0, 12)}</a>)}</div></div>)}</div> : <StatePanel title={tr(locale, "未发现明显异常", "No notable issues")} body={tr(locale, "当前规则没有识别到可定位的问题。", "No deterministic issue was detected.")} />}</section>
    ...
    <section className="at-card"><div className="at-card-head"><div><h2>{tr(locale, "事件时间线", "Event timeline")} <Badge>{events.counts.matching}</Badge></h2><p>{tr(locale, `当前显示：${eventVisibilityLabel(locale, visibility)}`, `Showing: ${eventVisibilityLabel(locale, visibility)}`)}</p></div><div className="at-actions">{(["display", "hidden", "all"] as const).map(mode => <button key={mode} className={`at-button ${mode === visibility ? "primary" : ""}`} onClick={() => { const next = new URLSearchParams(route.query); mode === "display" ? next.delete("visibility") : next.set("visibility", mode); navigate(withQuery(route.path, next)); }}>{eventVisibilityLabel(locale, mode)}</button>)}</div></div>
...
export function RunCompareView(context: PageContext) {
  ...
  return <>
    <PageHead locale={context.locale} eyebrow={tr(context.locale, "运行对比", "REGRESSION COMPARISON")} title={tr(context.locale, "Run 对比", "Run comparison")} body={tr(context.locale, "以第一个 Run 为基线比较状态、耗时、Token 与事件回归。", "Compare status, duration, tokens, and event regressions against the first run.")} />
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "运行指标", "Run metrics")} <Badge>{resource.data.regressionCount}</Badge></h2></div><div className="at-table-wrap"><table className="at-table"><thead><tr><th>Run</th><th>{tr(context.locale, "状态", "Status")}</th><th className="num">{tr(context.locale, "耗时", "Duration")}</th><th className="num">Token</th><th className="num">{tr(context.locale, "成本", "Cost")}</th></tr></thead><tbody>{resource.data.runs.map(run => <tr key={run.id}><td><strong className="at-run-name">{run.name}</strong><div className="at-subtle at-mono">{run.id}</div></td><td><StatusBadge status={run.status} locale={context.locale} /></td><td className="num">{formatDuration(run.durationMs)}</td><td className="num">{fmt(run.totalTokens)}</td><td className="num">{formatMoney(run.costUsd)}</td></tr>)}</tbody></table></div></section>
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "事件差异", "Event differences")}</h2></div><div className="at-list">{resource.data.eventDiffs.map((diff, index) => <div className="at-list-item" key={`${diff.runId}-${diff.eventKey}-${index}`}><strong>{diff.name}</strong><p>{eventTypeLabel(context.locale, diff.type)} · {diff.changes.map(value => comparisonValueLabel(context.locale, value)).join(", ") || tr(context.locale, "无变化", "Unchanged")} · {diff.regressions.map(value => comparisonValueLabel(context.locale, value)).join(", ") || tr(context.locale, "无回归", "No regression")}</p></div>)}</div></section>
  </>;
}
...
export function AnalyticsView(context: PageContext) {
  ...
  return <>
    <PageHead locale={context.locale} eyebrow={tr(context.locale, "分析", "ANALYTICS")} title={tr(context.locale, "分析与预算", "Analytics and budgets")} body={tr(context.locale, "按来源、模型、项目和环境分析运行质量，并管理预算告警。", "Analyze run quality by source, model, project, and environment and manage budgets.")} />
    <div className="at-actions">{["model", "source", "project", "environment"].map(value => <button key={value} className={`at-button ${dimension === value ? "primary" : ""}`} onClick={() => context.navigate(`/analytics?dimension=${value}${context.locale === "en" ? "&lang=en" : ""}`)}>{dimensionLabel(context.locale, value)}</button>)}</div>
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "30 天分布", "30-day breakdown")}</h2></div><div className="at-table-wrap"><table className="at-table"><thead><tr><th>{dimensionLabel(context.locale, dimension)}</th><th className="num">Run</th><th className="num">Token</th><th className="num">{tr(context.locale, "成本", "Cost")}</th><th className="num">{tr(context.locale, "失败率", "Failure rate")}</th></tr></thead><tbody>{breakdown.groups.map(group => <tr key={group.key}><td><strong>{group.key || "-"}</strong></td><td className="num">{fmt(group.runCount)}</td><td className="num">{fmt(group.totalTokens)}</td><td className="num">{formatMoney(group.costUsd)}</td><td className="num">{(group.failureRate * 100).toFixed(1)}%</td></tr>)}</tbody></table></div></section>
    <div className="at-grid-2">
      <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "新建预算", "New budget")}</h2></div><form className="at-card-body at-form" onSubmit={event => void createBudget(event)}><div className="at-grid-2"><Field label={tr(context.locale, "名称", "Name")}><input className="at-control" name="name" required /></Field><Field label={tr(context.locale, "维度值", "Dimension value")}><input className="at-control" name="value" required /></Field><Field label={tr(context.locale, "维度", "Dimension")}><select className="at-control" name="dimension">{["project", "environment", "model", "source"].map(value => <option value={value} key={value}>{dimensionLabel(context.locale, value)}</option>)}</select></Field><Field label={tr(context.locale, "周期", "Period")}><select className="at-control" name="period"><option value="daily">{periodLabel(context.locale, "daily")}</option><option value="monthly">{periodLabel(context.locale, "monthly")}</option></select></Field><Field label={tr(context.locale, "最大成本 USD", "Max cost USD")}><input className="at-control" name="maxCostUsd" type="number" min="0" step="any" /></Field><Field label={tr(context.locale, "最大 Token", "Max tokens")}><input className="at-control" name="maxTokens" type="number" min="0" /></Field><Field label={tr(context.locale, "最大 Run", "Max runs")}><input className="at-control" name="maxRuns" type="number" min="0" /></Field></div><button className="at-button primary">{tr(context.locale, "创建预算", "Create budget")}</button></form></section>
      <ListCard locale={context.locale} title={tr(context.locale, "告警", "Alerts")} items={alerts.alerts.map(item => [item.budgetName, `${analyticsMetricLabel(context.locale, item.metric)}: ${fmt(item.actual)} / ${fmt(item.limit)}`])} />
    </div>
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "预算规则", "Budget rules")} <Badge>{budgets.budgets.length}</Badge></h2></div><div className="at-list">{budgets.budgets.length ? budgets.budgets.map(item => <div className="at-list-item at-list-row" key={item.id}><div><strong>{item.name}</strong><p>{dimensionLabel(context.locale, item.dimension)}:{item.value} · {periodLabel(context.locale, item.period)} · {budgetLimits(item, context.locale)}</p></div><button className="at-button danger" onClick={() => void deleteBudget(item.id)}><Trash2 size={14} />{tr(context.locale, "删除", "Delete")}</button></div>) : <StatePanel title={tr(context.locale, "暂无预算", "No budgets")} body={tr(context.locale, "创建预算规则后会在这里显示。", "Create a budget to see it here.")} />}</div></section>
  </>;
}
...
export function SandboxView(context: PageContext) {
  ...
  return <>
    <PageHead locale={context.locale} eyebrow={tr(context.locale, "安全回放", "SAFE REPLAY")} title={tr(context.locale, "安全回放与调试沙箱", "Safe replay sandbox")} body={tr(context.locale, "在禁网、临时文件系统和模拟工具策略下回放事件。", "Replay events with network disabled, temporary storage, and mocked tools.")} />
    <TelemetryStrip items={[[tr(context.locale, "网络", "Network"), tr(context.locale, "禁用", "Disabled"), ""], [tr(context.locale, "工具执行", "Tool execution"), tr(context.locale, "仅模拟", "Mock only"), ""], [tr(context.locale, "文件系统", "Filesystem"), tr(context.locale, "临时", "Temporary"), ""], [tr(context.locale, "任务", "Tasks"), fmt(tasks.tasks.length), ""]]} />
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "载入来源 Run", "Load source run")}</h2></div><form className="at-card-body at-actions" onSubmit={load}><input className="at-control" style={{ maxWidth: 640 }} name="runId" defaultValue={runId} required placeholder="run-id" /><button className="at-button primary">{tr(context.locale, "载入", "Load")}</button>{run ? <button className="at-button" type="button" onClick={() => context.navigate(`/runs/${encodeURIComponent(runId)}`)}>{tr(context.locale, "查看详情", "View details")}</button> : null}</form></section>
    {run && events ? <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "创建回放", "Create replay")}</h2><Badge>{normalizeRunTitle(run.name)}</Badge></div><form className="at-card-body at-form" onSubmit={event => void replay(event)}><div className="at-grid-3"><Field label={tr(context.locale, "事件", "Event")}><select className="at-control" name="eventId">{events.events.map(item => <option value={item.id} key={item.id}>{eventTypeLabel(context.locale, item.type)} · {eventNameLabel(context.locale, item.name)}</option>)}</select></Field><Field label={tr(context.locale, "延迟毫秒", "Delay ms")}><input className="at-control" name="delayMs" type="number" defaultValue="0" /></Field><Field label={tr(context.locale, "超时毫秒", "Timeout ms")}><input className="at-control" name="timeoutMs" type="number" defaultValue="5000" /></Field></div><div className="at-grid-2"><Field label={tr(context.locale, "覆盖输入 JSON（可选）", "Override input JSON (optional)")}><textarea className="at-control at-mono" name="input" /></Field><Field label={tr(context.locale, "模拟输出 JSON（可选）", "Mock output JSON (optional)")}><textarea className="at-control at-mono" name="mockOutput" /></Field></div><label className="at-checkbox"><input type="checkbox" name="simulateError" /> {tr(context.locale, "模拟错误", "Simulate error")}</label><button className="at-button primary">{tr(context.locale, "开始回放", "Start replay")}</button></form></section> : null}
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "回放任务", "Replay tasks")} <Badge>{tasks.tasks.length}</Badge></h2></div><div className="at-list">{tasks.tasks.length ? tasks.tasks.map(item => <div className="at-list-item" key={item.id}><div className="at-list-row"><div><div className="at-actions"><StatusBadge status={item.status} locale={context.locale} /><strong className="at-mono">{item.id}</strong></div><p>{item.sourceRunId} · {item.sourceEventId} · {formatDate(item.createdAt, context.locale)}</p>{item.error ? <p className="at-error-text">{item.error}</p> : null}</div><div className="at-actions">{item.replayRunId ? <><button className="at-button" onClick={() => context.navigate(`/runs/${encodeURIComponent(item.replayRunId!)}`)}>{tr(context.locale, "查看生成 Run", "View replay run")}</button><button className="at-button" onClick={() => context.navigate(`/runs/compare?ids=${encodeURIComponent(`${item.sourceRunId},${item.replayRunId}`)}`)}>{tr(context.locale, "对比", "Compare")}</button></> : null}{item.status === "queued" || item.status === "running" ? <button className="at-button danger" onClick={() => void cancelReplay(item.id)}>{tr(context.locale, "取消", "Cancel")}</button> : null}</div></div></div>) : <StatePanel title={tr(context.locale, "暂无回放任务", "No replay tasks")} body={tr(context.locale, "创建任务后会在这里显示。", "Created replays appear here.")} />}</div></section>
  </>;
}
...
export function MaintenanceView(context: PageContext) {
  ...
  return <>
    <PageHead locale={context.locale} eyebrow={tr(context.locale, "维护", "MAINTENANCE")} title={tr(context.locale, "维护与隐私", "Maintenance and privacy")} body={tr(context.locale, "管理本地存储、隐私脱敏、保留策略和删除墓碑。", "Manage local storage, privacy redaction, retention, and tombstones.")} />
    <TelemetryStrip items={Object.entries(stats).slice(0, 4).map(([key, value]) => [storageMetricLabel(context.locale, key), typeof value === "number" ? fmt(value) : String(value ?? "-"), ""])} />
    <div className="at-grid-2">
      <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "隐私规则", "Privacy rules")}</h2></div><form className="at-card-body at-form" onSubmit={event => void savePrivacy(event)}><Field label={tr(context.locale, "敏感字段", "Sensitive keys")}><textarea className="at-control" name="sensitiveKeys" defaultValue={privacy.sensitiveKeys.join("\n")} /></Field><Field label={tr(context.locale, "替换文本", "Replacement")}><input className="at-control" name="replacement" defaultValue={privacy.replacement} /></Field><button className="at-button primary">{tr(context.locale, "保存", "Save")}</button></form></section>
      <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "按日期清理", "Prune by date")}</h2></div><form className="at-card-body at-form" onSubmit={event => void pruneRuns(event)}><Field label={tr(context.locale, "删除此日期之前", "Delete before")}><input className="at-control" type="date" name="before" defaultValue={defaultBefore} required /></Field><div className="at-actions"><label className="at-checkbox"><input type="checkbox" name="success" defaultChecked /> {statusLabel(context.locale, "success")}</label><label className="at-checkbox"><input type="checkbox" name="error" defaultChecked /> {statusLabel(context.locale, "error")}</label><label className="at-checkbox"><input type="checkbox" name="keepTombstones" defaultChecked /> {tr(context.locale, "保留墓碑", "Keep tombstones")}</label></div><button className="at-button danger">{tr(context.locale, "执行清理", "Prune runs")}</button></form></section>
    </div>
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "数据库操作", "Database operations")}</h2></div><div className="at-card-body at-actions"><button className="at-button" onClick={() => void context.client.send("/maintenance/compact", "POST", {}).then(resource.reload)}><Database size={14} />{tr(context.locale, "压缩数据库", "Compact database")}</button><button className="at-button" onClick={resource.reload}><RefreshCw size={14} />{tr(context.locale, "刷新统计", "Refresh stats")}</button></div></section>
    <section className="at-card"><div className="at-card-head"><h2>{tr(context.locale, "删除墓碑", "Deletion tombstones")} <Badge>{tombstones.tombstones.length}</Badge></h2></div><div className="at-list">{tombstones.tombstones.map(item => <div className="at-list-item at-list-row" key={item.runId}><div><strong>{item.runId}</strong><p>{formatDate(item.deletedAt, context.locale)}</p></div><button className="at-button" onClick={() => void context.client.send(`/runs/${encodeURIComponent(item.runId)}/tombstone`, "DELETE").then(resource.reload)}><RotateCcw size={14} />{tr(context.locale, "恢复 ID", "Restore ID")}</button></div>)}</div></section>
  </>;
}
...
function statusLabel(locale: DashboardLocale, status: string) {
  const labels: Record<string, [string, string]> = {
    success: ["成功", "Success"],
  ...
  return tr(locale, ...(labels[status] || [status, status]));
}

function eventVisibilityLabel(locale: DashboardLocale, visibility: string) {
  const labels: Record<string, [string, string]> = {
    display: ["主要事件", "Primary"],
    hidden: ["隐藏事件", "Hidden"],
    all: ["全部事件", "All"]
  };
  return tr(locale, ...(labels[visibility] || [visibility, visibility]));
}

function dimensionLabel(locale: DashboardLocale, dimension: string) {
  const labels: Record<string, [string, string]> = {
    model: ["模型", "Model"],
    source: ["来源", "Source"],
    project: ["项目", "Project"],
  ...
  return tr(locale, ...(labels[dimension] || [dimension, dimension]));
}

function periodLabel(locale: DashboardLocale, period: string) {
  return period === "daily" ? tr(locale, "每日", "Daily") : tr(locale, "每月", "Monthly");
}

function insightKindLabel(locale: DashboardLocale, kind: string) {
  const labels: Record<string, [string, string]> = {
    repeated_action: ["重复操作", "Repeated action"],
    retry_loop: ["重试循环", "Retry loop"],
    slow_step: ["慢步骤", "Slow step"],
    token_hotspot: ["Token 热点", "Token hotspot"],
    failure_cascade: ["故障级联", "Failure cascade"]
  };
  return tr(locale, ...(labels[kind] || [kind, kind]));
}

function severityLabel(locale: DashboardLocale, severity: string) {
  const labels: Record<string, [string, string]> = {
    info: ["提示", "Info"], warning: ["警告", "Warning"], error: ["错误", "Error"]
  };
  return tr(locale, ...(labels[severity] || [severity, severity]));
}

function storageMetricLabel(locale: DashboardLocale, metric: string) {
  const labels: Record<string, [string, string]> = {
    databasePath: ["数据库路径", "Database path"],
    databaseBytes: ["数据库大小", "Database size"],
    runs: ["Run 数量", "Runs"],
    events: ["事件数量", "Events"],
    usageSessions: ["用量会话", "Usage sessions"],
    tombstones: ["删除墓碑", "Tombstones"]
  };
  return tr(locale, ...(labels[metric] || [metric, metric]));
}

function surfaceLabel(locale: DashboardLocale, surface: string) {
  const labels: Record<string, [string, string]> = {
    cli: ["命令行", "CLI"], local: ["本地", "Local"], unknown: ["未知", "Unknown"],
    preview: ["预览", "Preview"], terminal: ["终端", "Terminal"], desktop: ["桌面端", "Desktop"], web: ["Web 端", "Web"]
  };
  return surface.split("/").map(value => tr(locale, ...(labels[value.trim()] || [value.trim(), value.trim()]))).join(" / ");
}

function sourceLabel(locale: DashboardLocale, source: string) {
  return source === "manual" ? tr(locale, "手动", "Manual") : source;
}

function comparisonValueLabel(locale: DashboardLocale, value: string) {
  const labels: Record<string, [string, string]> = {
    added: ["新增", "Added"], removed: ["移除", "Removed"], status: ["状态", "Status"],
    missing: ["缺失", "Missing"], duration: ["耗时", "Duration"], tokens: ["Token", "Tokens"]
  };
  return tr(locale, ...(labels[value] || [value, value]));
}

function analyticsMetricLabel(locale: DashboardLocale, metric: string) {
  const labels: Record<string, [string, string]> = {
    cost: ["成本", "Cost"], tokens: ["Token", "Tokens"], runs: ["Run", "Runs"]
  };
  return tr(locale, ...(labels[metric] || [metric, metric]));
}
...
function StatusBadge({ status, locale }: { status: string; locale: DashboardLocale }) { const tone = status === "success" || status === "completed" ? "success" : status === "error" || status === "timeout" ? "error" : "running"; return <span className={`at-badge ${tone}`}>{statusLabel(locale, status)}</span>; }
...
function ErrorState({ locale, error, retry }: { locale: DashboardLocale; error?: string; retry: () => void }) { return <div className="at-state"><div><AlertCircle className="at-error-text" size={28} /><h2>{tr(locale, "页面加载失败", "Page failed to load")}</h2><p>{error || tr(locale, "无法连接本地 Collector", "Failed to reach the local Collector")}</p><button className="at-button primary" onClick={retry}><RefreshCw size={14} />{tr(locale, "重试", "Retry")}</button></div></div>; }
...
function ListCard({ locale, title, items }: { locale: DashboardLocale; title: string; items: Array<[string, string]> }) { return <section className="at-card"><div className="at-card-head"><h2>{title}</h2></div><div className="at-list">{items.length ? items.map(([name, body], index) => <div className="at-list-item" key={`${name}-${index}`}><strong>{name}</strong><p>{body}</p></div>) : <StatePanel title={tr(locale, "暂无数据", "No data")} body={tr(locale, "当前没有可显示的记录。", "There are no records to display.")} />}</div></section>; }
...
function budgetLimits(item: AnalyticsBudget, locale: DashboardLocale) { return [item.maxCostUsd == null ? "" : formatMoney(item.maxCostUsd), item.maxTokens == null ? "" : `${fmt(item.maxTokens)} Token`, item.maxRuns == null ? "" : `${fmt(item.maxRuns)} Run`].filter(Boolean).join(" · ") || tr(locale, "未设置限制", "No limits"); }
