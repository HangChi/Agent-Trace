...
4. 确认关闭窗口会隐藏到托盘，托盘可重新打开窗口并显式退出。
5. 使用非 Agent-Trace 服务占用 `4319` 后启动，确认错误清晰且不会打开不可用窗口。
6. 运行 `pnpm desktop:build:win`，确认 `target/release/bundle/nsis` 生成安装器并完成安装/卸载检查。
