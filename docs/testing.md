...
| Tauri Desktop | `pnpm desktop:check:rust` | 静态 UI 契约、Rust Collector/Scanner 测试、桌面 crate 编译测试与工作区 Clippy。 |
...
2. 确认 Collector、静态 Dashboard 和 Scanner 生命周期；第二个实例应因固定 `4319` 端口冲突而拒绝启动。
3. 确认关闭窗口会隐藏到托盘，托盘可重新打开窗口并显式退出。
4. 占用 `4319` 端口后启动，确认错误清晰且不会打开不可用窗口。
5. 运行 `pnpm desktop:build:win`，确认 `target/release/bundle/nsis` 生成安装器并完成安装/卸载检查。
