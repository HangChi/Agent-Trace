...
## 2026-07-15：P0 可扩展性与本地治理

- SDK 公开通用 `traceStep`，通过异步上下文自动维护嵌套父 Event，并支持 Run 级组织 metadata。
- Run 增加项目、环境、版本、标签、备注和收藏编辑，以及对应列表筛选。
- 新增维护与隐私控制中心，复用容量、清理、压缩和墓碑恢复能力，并持久化写入前敏感字段脱敏规则。
