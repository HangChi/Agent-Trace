...
| PATCH | `/runs/:id/organization` | 更新项目、环境、版本、标签、备注和收藏状态 |
...
| POST | `/v1/traces` | 接收通用 OTLP/HTTP JSON Trace |
| POST | `/integrations/otlp/v1/traces` | 通用 OTLP Trace 显式接入路径 |
...
| GET | `/analytics/breakdown` | 按项目、环境、模型或来源聚合 |
| GET | `/analytics/budgets` | 查询预算 |
| POST | `/analytics/budgets` | 创建预算 |
| DELETE | `/analytics/budgets/:id` | 删除预算 |
| GET | `/analytics/alerts` | 查询当前预算告警 |
| GET | `/evaluations/datasets` | 查询评测集 |
| POST | `/evaluations/datasets` | 创建评测集 |
| GET | `/evaluations/datasets/:id` | 查询评测用例、结果与质量报告 |
| POST | `/evaluations/datasets/:id/cases` | 添加评测用例 |
| POST | `/evaluations/results` | 记录 Run 加权评分 |
| GET | `/sandbox/replays` | 查询安全回放任务 |
| POST | `/sandbox/replays` | 创建安全回放任务 |
| GET | `/sandbox/replays/:id` | 查询单个回放任务 |
| DELETE | `/sandbox/replays/:id` | 取消排队中或运行中的回放任务 |
| DELETE | `/runs/:id` | 删除单个 Run |
| DELETE | `/runs/:id/tombstone` | 解除删除墓碑，允许同 ID 再次采集 |
| GET | `/maintenance/storage` | 查询数据库容量与行数 |
| GET | `/maintenance/tombstones` | 查询最近删除的 Run 墓碑 |
| GET | `/maintenance/privacy` | 查询写入前字段脱敏规则 |
| PUT | `/maintenance/privacy` | 更新写入前字段脱敏规则 |
...
### `PATCH /runs/:id/organization`

更新 Run 的 `project`、`environment`、`version`、`tags`、`note` 与 `favorite` 元数据。文本字段传 `null` 或空字符串可清除；标签最多 20 个，备注最多 2000 字符。该接口只修改组织信息，不改变采集来源和运行状态。
...
Run 对比还会按“Event 类型 + 名称 + 出现次序”匹配事件。新增失败、基准事件缺失、耗时或 Token 增长超过 20% 会标记为回归。

`GET /analytics/breakdown?dimension=project&days=30` 支持 `project`、`environment`、`model`、`source` 四个维度。预算使用 UTC 日/月窗口，可设置成本、Token、Run 数上限；`GET /analytics/alerts` 根据当前数据实时返回超限项。

评测集保存评分维度权重和用例；`POST /evaluations/results` 对同一用例与 Run 的组合执行 upsert，并计算 0–1 的加权质量分。

通用 OTLP Exporter 可向 `/v1/traces` 或 `/integrations/otlp/v1/traces` 发送 `resourceSpans`。Trace ID 映射为 Run，Span 映射为带父子关系的 Event，GenAI 语义字段映射模型与 Token。
...
- `GET /maintenance/tombstones?limit=50` 返回最近删除的 Run ID、删除时间和原因，最多 200 条；使用既有 `DELETE /runs/:id/tombstone` 允许同 ID 再次采集。
...
- `GET /maintenance/privacy` 返回 `{ "sensitiveKeys": [], "replacement": "[REDACTED]" }`。`PUT /maintenance/privacy` 替换该配置；后续 Run/Event/Transcript 写入会递归检查字段名（不区分大小写），命中后在持久化前替换字段值。规则不会追溯修改已经存储的数据。
...
`POST /sandbox/replays` 接受 `sourceRunId`、`sourceEventId`，以及可选的 `input`、`mockOutput`、`simulateError`、`delayMs` 和 `timeoutMs`。成功时返回 `202` 和排队任务；任务完成后会生成 ID 为 `replay:<taskId>` 的新 Run。`timeoutMs` 范围为 100–30000 毫秒。

回放只启动仓库内置的固定 Mock Worker，不接受 Shell、脚本或任意用户代码，也不调用真实工具。Worker 使用净化环境变量和独立临时目录；超时或取消后会等待子进程退出并清理目录。这里的“网络禁用”来自固定 Worker 没有网络能力，并非操作系统级网络隔离。任务和生成的 Run/Event 会保存在本地 SQLite。`DELETE /sandbox/replays/:id` 是幂等操作；任务已经进入终态时返回当前任务和 `200`，避免停止请求与任务完成之间的竞态升级为页面错误。
